...
public class PDFTableOfContentsExtractor {
    ...
    public static void extractTableOfContentsToJson(String pdfFilePath, String jsonOutputPath) {
        try (PDDocument document = PDDocument.load(new File(pdfFilePath))) {
            ...
            if (outline != null) {
                ...
                writeOutlineItemToJson(outline.getFirstChild(), writer, 1);
                writer.write("  ]\n");
                writer.write("}\n");
                writer.close();
            } else {
    ...
    public static void writeOutlineItemToJson(PDOutlineItem item, FileWriter writer, int level) throws IOException {
        if (item != null) {
            writer.write("    {\n");
            ...
            if (item.getNextSibling() != null) {
                writer.write("    },\n");
                writeOutlineItemToJson(item.getNextSibling(), writer, level);
            } else {
                writer.write("    }\n");
            }
            if (item.getFirstChild() != null) {
                writer.write(",\n");
                writeOutlineItemToJson(item.getFirstChild(), writer, level + 1);
            }
        }
    }
}